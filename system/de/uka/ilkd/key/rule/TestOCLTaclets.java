...
public class TestOCLTaclets extends TestCase {
    ...
    public void setUp() {
        ...
        x = (SortedSchemaVariable) SchemaVariableFactory.createVariableSV(
                new Name("x"), new GenericSort(new Name("AnySV")), false);
        initConf.varNS().add(x);
        acc = (SortedSchemaVariable) SchemaVariableFactory.createVariableSV(
                new Name("acc"), new GenericSort(new Name("AnyCollSV")), false);
        initConf.varNS().add(acc);
        nonGenX = (SortedSchemaVariable) SchemaVariableFactory
                .createVariableSV(new Name("nonGenX"), OclSort.OCLANY, false);
        initConf.varNS().add(nonGenX);
        nonGenAcc = (SortedSchemaVariable) SchemaVariableFactory
                .createVariableSV(new Name("nonGenAcc"), OclSort.OCLGENERIC,
                        false);
        initConf.varNS().add(nonGenAcc);
        e1 = (SortedSchemaVariable) SchemaVariableFactory.createTermSV(
                new Name("e1"), OclSort.OCLANY, false, true, false);
        initConf.varNS().add(e1);
        setSV = (SortedSchemaVariable) SchemaVariableFactory.createTermSV(
                new Name("setSV"), OclSort.SET_OF_OCLANY, false, true, false);
        initConf.varNS().add(setSV);
        g1 = (SortedSchemaVariable) SchemaVariableFactory.createTermSV(
                new Name("g1"), OclSort.OCLGENERIC, false, true, false);
        initConf.varNS().add(g1);
        g2 = (SortedSchemaVariable) SchemaVariableFactory.createTermSV(
                new Name("g2"), OclSort.OCLGENERIC, false, true, false);
        ...
        ocl_iterateSet_empty = (RewriteTaclet) parseTaclet("ocl_iterateSet_empty{\\find($iterate($empty_set, g1, \\bind( x; acc) g2)) "
                + "\\replacewith(g1)}");

        ocl_equals = (RewriteTaclet) parseTaclet("ocl_equals{\\find($equals(g1, g1)) \\replacewith($true)};");

        ocl_if_true = (RewriteTaclet) parseTaclet("ocl_if_true{\\find($if($true, g1, g2)) \\replacewith(g1)};");
    ...
    private KeYParser stringTermParser(String s) {
        ...
        return new KeYParser(ParserMode.TERM, new KeYLexer(new StringReader(s),
                null),
                "No file. Call of parser from parser/TestTermParser.java", tf,
                new Recoder2KeY(TacletForTests.services(), nss), TacletForTests
                        .services(), nss, new AbbrevMap(), provider);
    }
    ...
    private KeYParser stringTacletParser(String s) {
        Profile profile = Main.getInstance().mediator().getProfile();
        ProgramBlockProvider provider = profile.getProgramBlockProvider();
        return new KeYParser(ParserMode.TACLET, new KeYLexer(
                new StringReader(s), null),
                "No file. parser/TestTacletParser.stringTacletParser(" + s
                        + ")", tf, TacletForTests.services(), nss, provider);
    }

    public Taclet parseTaclet(String s) {
        try {
            KeYParser p = stringTacletParser(s);
            return p.taclet(SetAsListOfChoice.EMPTY_SET.add(new Choice(
                    new Name("Default"))));
        } catch (Exception e) {
            StringWriter sw = new StringWriter();
            PrintWriter pw = new PrintWriter(sw);
    ...
    public void test1() {
        ...
        assertEquals("ocl_equals applied to: $equals(\"set\", \"set\")",
                result, expected);
    }
    ...
    public void test2() {
        ...
        assertEquals("ocl_if applied to: $if($true, $false, $true)", result,
                expected);
    }
    ...
    public void test3() {
        ...
        LogicVariable this_elem = (LogicVariable) t.varsBoundHere(2)
                .getQuantifiableVariable(0);
        LogicVariable this_acc = (LogicVariable) t.varsBoundHere(2)
        ...
        TermSymbol and = (TermSymbol) initConf.funcNS()
                .lookup(new Name("$and"));
        Term andTerm2 = TermFactory.DEFAULT.createFunctionTerm(and, new Term[] {
                elemT, accT });
        // TermSymbol iterate = TacletForTests.funcLookup("$iterate");
        TermSymbol iterate = (TermSymbol) initConf.funcNS().lookup(
                new Name("$iterate"));
        ...
        var[2] = new ArrayOfQuantifiableVariable(new LogicVariable[] {
                this_elem, this_acc });
        Term iterateTerm = TermFactory.DEFAULT.createFunctionWithBoundVarsTerm(
                iterate, new Term[] { nilT, falseT, andTerm2 }, var);
        Term expected = TermFactory.DEFAULT.createFunctionTerm(and, new Term[] {
                trueT, iterateTerm });

        assertEquals(
                "ocl_iterate_step applied to: $iterate($insert_set($true, $empty_set), $false, "
                        + "\\bind(OclBoolean elem; OclBoolean accum) $and(elem, accum)",
                result, expected);
    }
    ...
    public void test4() {
        ...
        Term expected = parseTerm("$false");
        assertEquals(
                "ocl_iterateSet_empty applied to: $iterate($empty_set, $false, "
    ...
    public void test5() {
        index.tacletIndex().remove(
                index.tacletIndex().lookup("ocl_iterateSet_step"));
        ocl_iterateSet_step = (RewriteTaclet) parseTaclet("ocl_iterateSet_step{\\find($iterate($insert_set(e1,setSV), "
    ...
    private ListOfTacletApp findMatchingTaclets(Term t) {
        TermSymbol oclWrapper = (TermSymbol) initConf.funcNS().lookup(
                new Name("$OclWrapper"));
        ...
        // Matching of taclets
        PosInOccurrence pos = new PosInOccurrence(cf, PosInTerm.TOP_LEVEL
                .down(0), false);
        ListOfTacletApp list = index.getTacletAppAt(TacletFilter.TRUE, pos,
