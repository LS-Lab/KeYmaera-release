 ...
 * </ul>
 * Its methods <code>complete</code> or <code>sufficientlyComplete</code>
 * are used to determine if the information is complete or at least sufficient
 * (can be completed using meta variables) complete, so that is can be applied.
 */
public abstract class TacletApp implements RuleApp {
    ...
    protected static SVInstantiations resolveCollisionVarSV(Taclet taclet,
        ...
        HashMapFromLogicVariableToSchemaVariable collMap = new HashMapFromLogicVariableToSchemaVariable();
        IteratorOfEntryOfSchemaVariableAndInstantiationEntry it = insts
                .pairIterator();
        while (it.hasNext()) {
    ...
    private NewDepOnAnonUpdates getDependingOnModifies(SchemaVariable var) {
        ...
        while (it.hasNext()) {
            ...
            if (vc instanceof NewDepOnAnonUpdates
                ...
                assert result == null : "" + vc
                        + "is a duplicate dependency condition for "
                        + "schema variable " + var;
            ...
            while (it.hasNext()) {
                ...
                if (canUseMVAPriori(var)) {
                    GenericSortCondition c = GenericSortCondition
                            .forceInstantiation(((SortedSchemaVariable) var)
                                    .sort(), true);
                    if (c == null)
            ...
            if (var instanceof SortedSchemaVariable) {
                ...
                if (sv.sort() == ProgramSVSort.VARIABLE) {
                    ...
                    String proposal = VariableNameProposer.DEFAULT.getProposal(
                            app, sv, services, goal.node(), null);
                    app = app
                            .createSkolemConstant(proposal, sv, true, services);
                    ...
                    String proposal = VariableNameProposer.DEFAULT.getProposal(
                            this, sv, services, goal.node(), null);
                    final LogicVariable v = new LogicVariable(
                            new Name(proposal), getRealSort(sv, services));
        ...
        try {
            while (it.hasNext()) {
                ...
                final GenericSortCondition c = GenericSortCondition
                        .forceInstantiation(((SortedSchemaVariable) sv).sort(),
                                true);
                if (c != null)
                    insts = insts.add(c);
            }
        ...
        if (newVars != SetAsListOfMetavariable.EMPTY_SET) {
            ...
            while (mvIt.hasNext()) {
                ...
                if (mv.isTemporaryVariable()) {
                    ...
                    final Term newT = TermFactory.DEFAULT
                            .createFunctionTerm(newMV);
     ...
     *            or if it can be derived deterministically (e.g. by matching) (<code>interesting==false</code>)
     ...
     * is possible. If the resulting instantiation is valid a new taclet
    ...
    public boolean canUseMVAPriori(SchemaVariable p_var) {
        if (!p_var.isTermSV() || fixedVars.contains(p_var))
            return false;
        else {
