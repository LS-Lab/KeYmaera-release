...
//            }
	...
	// Sort equality has to be ensured before calling this method
        ...
        if (visitedOp instanceof SortedSchemaVariable
           ...
           Term[] neededsubs;
           try {
               neededsubs = neededSubs(newOp.arity());
           } catch(Exception e) {
               // the following code was used as a work around for some issue that showed up when dealing with games
               // when we parse taclets with games and there are meta variables around, we get here.
