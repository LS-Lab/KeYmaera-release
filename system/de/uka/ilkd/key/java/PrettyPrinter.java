...
public class PrettyPrinter {
    ...
    public void printStatementBlock(StatementBlock x)
        ...
        if (!(Main.getInstance().mediator().getProfile().getClass() == DLProfile.class)) {
            write("{");
        } else {
            writeIndentation(1, 0);
        }
    ...
    /**
     * FIXME dirty hack here. this should be another class
     * 
     * @param p
     */
    public void printDLProgramElement(DLProgramElement p) throws IOException {
        printHeader(p);
        markStart(0, p);
        if (p instanceof SymbolizedElement) {
            write(((SymbolizedElement) p).getSymbol());
        } else if (p instanceof NamedElement) {
            write(((NamedElement) p).getElementName().toString());
        } else {
            write(p.toString());
        }
     ...
     * @param p
     */
    public void printDLNonTerminalProgramElement(DLNonTerminalProgramElement p,
            boolean symbolChild0) throws IOException {
        printHeader(p);
        markStart(0, p);
        noLinefeed = false;
        PresentationInfoEntry e;
        if (symbolChild0) {
            e = PresentationInfo.INSTANCE.getEntry(p.getChildAt(0));
        } else {
            e = PresentationInfo.INSTANCE.getEntry(p);
        }
        int startIndex = (symbolChild0) ? 1 : 0;
        if (e != null) {
            switch (e.getLineBreak()) {
            case BEFORE:
                writeIndentation(1, getTotalIndentation());
            }
            switch (e.getFix()) {
            case INFIX:
                for (int i = startIndex; i < p.getChildCount(); i++) {
                    ...
                    PresentationInfoEntry eChild = PresentationInfo.INSTANCE
                            .getEntry(child);
                    boolean braces = true;
                    if (eChild == null) {
                        braces = false;
    ...
    public void printDLNonTerminalProgramElement(DLNonTerminalProgramElement p)
            throws IOException {
        printDLNonTerminalProgramElement(p, false);
    }
     ...
     * @param p
     */
    public void printCompoundTerm(DLNonTerminalProgramElement p, boolean infix)
            throws IOException {
        // printHeader(p);
        // markStart(0, p);
        //
        ...
        // markEnd(0, p);
        // printFooter(p);
        printDLNonTerminalProgramElement(p, true);
    }
     ...
     * @param p
     */
    public void printFunctionSymbol(de.uka.ilkd.key.dl.model.Function p)
            throws IOException {
        printHeader(p);
        markStart(0, p);
     ...
     * @param p
     */
    public void printDiffSystem(DLNonTerminalProgramElement p)
            throws IOException {
        printHeader(p);
        markStart(0, p);
        write("{");
     ...
     * @param p
     */
    public void printStar(DLNonTerminalProgramElement p) throws IOException {
        printHeader(p);
        markStart(0, p);
        write("(");
        writeElement(p.getChildAt(0));
     ...
     * @param p
     */
    public void printRandomAssign(RandomAssign p) throws IOException {
        printHeader(p);
        markStart(0, p);
        writeElement(p.getChildAt(0));
        write(" := *");
     ...
     * @param p
     */
    public void printDLVariableDeclaration(
            de.uka.ilkd.key.dl.model.VariableDeclaration p)
            throws IOException {
        printHeader(p);
        markStart(0, p);
        writeElement(p.getType());
        write(" ");
        String comma = "";
        for(int i = 1; i < p.getChildCount(); i++) {
            write(comma);
     ...
     * @param t
     */
    public void printDLIf(IfStatement p) throws IOException {
        printHeader(p);
        markStart(0, p);
        write("if(");
        writeElement(p.getExpression());
        write(")");
        writeIndentation(1, getTotalIndentation());
        write("then");
        changeLevel(1);
        writeIndentation(1, getTotalIndentation());
        writeElement(p.getThen());
        changeLevel(-1);
        if (p.getElse() != null) {
            writeIndentation(1, getTotalIndentation());
            write("else");
     ...
     * @param existsImpl
     */
    public void printExists(Exists p) throws IOException {
        printHeader(p);
        markStart(0, p);
     ...
     * @param existsImpl
     */
    public void printForall(Forall p) throws IOException {
        printHeader(p);
        markStart(0, p);
