    ...
    private static int easySDP(int[] blockSizes, int k, double[] blockmatrixC,
        ...
        if (DLOptionBean.INSTANCE.isCsdpForceInternal()) {
            try {
                ...
                return CSDP.easySDP(blockSizes, k, blockmatrixC, a, constraints,
                                    constant_offset, blockmatrixpX, py, blockmatrixpZ,
                                    ppobj, pdobj);
            } catch (Exception e) {
            ...
            try {
                assert blockSizes.length == 1;
                return CSDPBinaryInterface.easySDP(blockSizes[0], k, blockmatrixC, a, constraints,
                                                   constant_offset, blockmatrixpX, py, blockmatrixpZ,
                                                   ppobj, pdobj);
            } catch (IOException e) {
                e.printStackTrace();
