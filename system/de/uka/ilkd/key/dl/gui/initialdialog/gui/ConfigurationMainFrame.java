 ...
 */
public class ConfigurationMainFrame implements ActionListener {

    public static final ConfigurationMainFrame INSTANCE = new ConfigurationMainFrame();
    ...
    private LinkedHashMap<String, List<PropsSettingsBeans>> groupMap;

    ConfigurationMainFrame() {
        pathFrame = new JFrame("KeYmaera Settings");
        groupMap = new LinkedHashMap<String, List<PropsSettingsBeans>>();

        for (EPropertyConfigurations k : EPropertyConfigurations.values()) {
            PropsSettingsBeans editor = new PropsSettingsBeans();
            editor.setPathPane(k.getLabel(), k.getEditorClass(), k
                    .getConverterClass(), k.getConfigFile(), k.getKey());
            List<PropsSettingsBeans> editorsInGroup = groupMap
                    .get(k.getGroup());
            if (editorsInGroup == null) {
                editorsInGroup = new LinkedList<PropsSettingsBeans>();
    ...
    private JPanel getGroupPaths(List<PropsSettingsBeans> group, String title) {
        ...
        ListIterator<PropsSettingsBeans> iter = group.listIterator();
        int y = 0;
        while (iter.hasNext()) {
            c.gridy = y++;
    ...
    private void writePropertyChanges() {
        for (List<PropsSettingsBeans> property : groupMap.values()) {
            ListIterator<PropsSettingsBeans> iter = property.listIterator();
            while (iter.hasNext()) {
    ...
    public Boolean getCheckboxState() {
        ...
        checkboxState = (Boolean) PropsSettingsBeans.INSTANCE.getValueOfKey(EPropertyConfigurations.CHECKBOX_PROPERTY.getConfigFile(),
