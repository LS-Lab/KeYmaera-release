...
import de.uka.ilkd.key.dl.gui.dialogwithsidepane.gui.options.FileExistenceVerification;
import de.uka.ilkd.key.dl.gui.dialogwithsidepane.gui.options.WriteProperties;
public class PropertyGroupCard  implements ActionListener{
	...
	void setPropertiesFields(List<PropertyConfigurationBeans> group,
		...
		ListIterator<PropertyConfigurationBeans> iter = this.group.listIterator();		
		while (iter.hasNext()) {
		    c.gridy = y++;
		    beans = iter.next(); 
		    beans.getPropertyEditor().addPropertyChangeListener(new PropertyChangeListener() {
		...
		TitledBorder border = new TitledBorder(BorderFactory.createEtchedBorder(),title,2,0);
    ...
    public Boolean isNextAvailable(int CardsAvailable) {
	
	if (index < (CardsAvailable -1) && index >=-1)
	    return true;
	else
	    return false;

    }
    public Boolean isPreviousAvailable(int CardsAvailable) {
	
	if (index > 0 && index < CardsAvailable)
	    return true;
	else
	    return false;
