...
vardecl returns [ DLProgram a ] scope { List<CommonTree> decls; } @init { $vardecl::decls = new ArrayList<CommonTree>(); }: 
vt = WORD (var = WORD { $vardecl::decls.add(var);} )+ { a = tf.createVariableDeclaration(vt, $vardecl::decls); }
;
