...
public class TestTermParserOCL extends TestCase {
	...
	private TermSymbol iterate, true_, false_, cons, nil, and, forAll,
		oclWrapper;
	...
	private KeYParser stringTermParser(String s) {
		...
		return new KeYParser(ParserMode.TERM,
							 new KeYLexer(new StringReader(s), null),
							 "No file. Call of parser from parser/TestTermParser.java",
							 tf, new Recoder2KeY(TacletForTests.services(),
												 nss),
							 TacletForTests.services(), nss, new AbbrevMap(),
							 p.getProgramBlockProvider());
	}
	...
	public void test2() {
		Term tParse =
			parseTerm
			("$forAll($insert_set($true, $insert_set($false, $empty_set))," +
			 "\\bind OclBoolean s; $and($true, s))");
		LogicVariable this_s =
			(LogicVariable) tParse.varsBoundHere(1).
		...
		var[1] = new ArrayOfQuantifiableVariable(new LogicVariable[] {
												 this_s});
		Term tS = tf.createVariableTerm(this_s);
		Term t1Cons =
			tf.createFunctionTerm(cons, new Term[]{ falseTerm, nilTerm });
		Term tCons =
			tf.createFunctionTerm(cons, new Term[]{ trueTerm, t1Cons });
		Term tAnd = tf.createFunctionTerm(and, trueTerm, tS);
		Term term =
			tf.createFunctionWithBoundVarsTerm(forAll, new Term[]{ tCons,
											   tAnd }, var);
		assertEquals
			("parse $forAll($insert_set($true, $insert_set($false, $empty_set)),"
			 + "\\bind OclBoolean s; $and($true, s))", term, tParse);
	}
	...
	public void test3() {
		Term tParse =
			parseTerm
			("$iterate($insert_set($true, $insert_set($false, $empty_set))," +
			 "$false, \\bind(OclBoolean s; OclBoolean acc) $and(s, acc))");
		LogicVariable this_s =
			(LogicVariable) tParse.varsBoundHere(2).
			getQuantifiableVariable(0);
		...
		Term term = tf.createFunctionWithBoundVarsTerm(iterate,
													   ...
													   var);
		assertEquals
			("parse $iterate($insert_set($true, $insert_set($false, $empty_set)),"
			 + "$false, \\bind(OclBoolean s; OclBoolean acc) $and(s, acc))",
			 term, tParse);
	}
