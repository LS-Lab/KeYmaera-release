 ...
 * separated by their sorts. A formula (and just a formula) must have the sort
 * {@link Sort#FORMULA}. Terms of a different sort are terms in the customary
 * logic sense. A term of sort formula is allowed exact there, where a formuala
 * in logic is allowed to appear, same for terms of different sorts. Some words
 * about other design decisions:
 * <ol>
 * <li> terms are immutable, this means after a term object is created, it
 * cannot be changed. The advantage is that we can use term sharing and saving a
 * lot of memory space. </li>
 * <li> Term has to be created using the {@link TermFactory} and <emph>not</emph>
 * by using the constructors itself. </li>
 * <li> Term is subclassed, but all subclasses have to be package private, so
 * that all other classes except {@link TermFactory} know only the class Term
 * and its interface. Even most classes of the logic package. </li>
...
public abstract class Term implements SVSubstitute {
	...
	public static final ArrayOfQuantifiableVariable EMPTY_VAR_LIST = new ArrayOfQuantifiableVariable(
	 ...
	 */
	protected SetOfQuantifiableVariable freeVars = SetAsListOfQuantifiableVariable.EMPTY_SET;
	...
	private void determineFreeVars() {
		for (int i = 0, ar = arity(); i < ar; i++) {
			...
			for (int j = 0, sz = varsBoundHere(i).size(); j < sz; j++) {
				subFreeVars = subFreeVars.remove(varsBoundHere(i)
	...
	/**
	 * this method has to be called by subclasses after they have assigned the
	 * subterms of the term. The method then can determine whether the operator
	 * and the subterms are rigid and put this information in a cache.
	 * 
	 * the implementation of this method uses the fact that quantifiable
	 * variables are rigid; otherwise, the results are too pessimistic (but
	 * still valid)
	 */
	 ...
	 * 
	 * @param o
	 *            the Object to be compared with this term
	 * @return true iff the given Term has the same values in operator, sort,
	 *         arity, varsBoundHere and javaBlock as this object.
	 */
	public boolean equals(Object o) {
		...
		if (!(t.hashCode() == hashCode() && sort() == t.sort()
				&& arity() == t.arity() && op().equals(t.op()) && javaBlock()
				.equals(t.javaBlock()))) {
			return false;
		}
	 ...
	 * 
	 * @return true iff the given Term has the same values in operator, sort,
	 *         arity, varsBoundHere and javaBlock as this object modulo bound
	 *         renaming
	 */
	...
	/**
	 * the visitor is handed through till the bottom of the tree and then it
	 * walks upwards while at each upstep the method visit of the visitor is
	 * called
	 * 
	 * @param visitor
	 *            the Visitor
	...
	/**
	 * The primary diamond in this formula. Note that the default implementation
	 * is the same as <code>javaBlock()</code> but the semantics is different.
	 * See <code>SimultaneousUpdateTerm</code>.
	 */
	...
	/**
	 * @return true iff all subterms of this term are rigid according to the
	 *         value the method "isRigid" returns for them (this does not imply
	 *         this term to be rigid itself!)
	 */
	 ...
	 * given i.e. the terms value may be the same in all states or not. (safe
	 * approximation)
	 * 
	 * @return false if the value of this term may be changed by modalities
	 *         (otherwise, however, the result may also be false)
	 */
	 ...
	 * the variables in <code>freeVars</code> are bound by some quantifier
	 * above)
	 * 
	 ...
	 * 
	 * @param nr
	 *            the int specifying the <tt>nr</tt>-th direct subterm
	 */
	 ...
	 * 
	 * @param pos
	 *            the position of the wanted subterm
