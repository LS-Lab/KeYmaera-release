...
public abstract class ProgramSVSort extends PrimitiveSort {
	...
	private static HashMap<Name, ProgramSVSort> name2sort = new HashMap<Name, ProgramSVSort>(
			60);
	...
	public static final ProgramSVSort SIMPLEANYJAVATYPEEXPRESSION = new SimpleExpressionSpecialPrimitiveTypeSort(
			"AnyJavaTypeExpression", new PrimitiveType[] {
					PrimitiveType.JAVA_BYTE, PrimitiveType.JAVA_SHORT,
	...
	public static final ProgramSVSort IMPLICITCLINIT = new ImplicitFieldSort(
			new Name("ImplicitClassInitialized"),
			ImplicitFieldAdder.IMPLICIT_CLASS_INITIALIZED, true);

	public static final ProgramSVSort IMPLICITINITINPROGRESS = new ImplicitFieldSort(
			new Name("ImplicitClassInitializationInProgress"),
			ImplicitFieldAdder.IMPLICIT_CLASS_INIT_IN_PROGRESS, true);

	public static final ProgramSVSort IMPLICITERRONEOUS = new ImplicitFieldSort(
			new Name("ImplicitClassErroneous"),
			ImplicitFieldAdder.IMPLICIT_CLASS_ERRONEOUS, true);

	public static final ProgramSVSort IMPLICITPREPARED = new ImplicitFieldSort(
			new Name("ImplicitClassPrepared"),
			ImplicitFieldAdder.IMPLICIT_CLASS_PREPARED, true);

	public static final ProgramSVSort IMPLICITNEXTTOCREATE = new ImplicitFieldSort(
			new Name("ImplicitNextToCreate"),
			ImplicitFieldAdder.IMPLICIT_NEXT_TO_CREATE, true);

	public static final ProgramSVSort IMPLICITCREATED = new ImplicitFieldSort(
			new Name("ImplicitCreated"), ImplicitFieldAdder.IMPLICIT_CREATED,
			true);

	public static final ProgramSVSort IMPLICITENLOSINGTHIS = new ImplicitFieldSort(
			new Name("ImplicitEnclosingThis"),
			ImplicitFieldAdder.IMPLICIT_ENCLOSING_THIS, true);

	public static final ProgramSVSort IMPLICITTRAINITIALIZED = new ImplicitFieldSort(
			new Name("ImplicitTraInitialized"),
			ImplicitFieldAdder.IMPLICT_ARRAY_TRA_INITIALIZED, true);

	public static final ProgramSVSort IMPLICITTRANSACTIONCOUNTER = new ImplicitFieldSort(
			new Name("ImplicitTransactionCounter"),
	...
	public static final ProgramSVSort JCMAKETRANSIENTARRAY = new MethodNameReferenceSort(
			new Name("makeTransientArray"), new String[] {
					"jvmMakeTransientBooleanArray",
					"jvmMakeTransientByteArray", "jvmMakeTransientShortArray",
					"jvmMakeTransientObjectArray" },
			"de.uka.ilkd.key.javacard.KeYJCSystem", SLListOfName.EMPTY_LIST
					.append(new Name("byte")).append(new Name("short")));

	public static final ProgramSVSort JCARRAYCOPY = new SpecificMethodNameSort(
			new ProgramElementName("jvmArrayCopy"));
	public static final ProgramSVSort JCARRAYCOPYNONATOMIC = new SpecificMethodNameSort(
			new ProgramElementName("jvmArrayCopyNonAtomic"));
	public static final ProgramSVSort JCARRAYFILLNONATOMIC = new SpecificMethodNameSort(
			new ProgramElementName("jvmArrayFillNonAtomic"));
	public static final ProgramSVSort JCARRAYCOMPARE = new SpecificMethodNameSort(
			new ProgramElementName("jvmArrayCompare"));

	public static final ProgramSVSort JCMAKESHORT = new SpecificMethodNameSort(
			new ProgramElementName("jvmMakeShort"));
	public static final ProgramSVSort JCSETSHORT = new SpecificMethodNameSort(
			new ProgramElementName("jvmSetShort"));

	public static final ProgramSVSort JCISTRANSIENT = new SpecificMethodNameSort(
			new ProgramElementName("jvmIsTransient"));
	public static final ProgramSVSort JCBEGINTRANSACTION = new SpecificMethodNameSort(
			new ProgramElementName("jvmBeginTransaction"));
	public static final ProgramSVSort JCCOMMITTRANSACTION = new SpecificMethodNameSort(
			new ProgramElementName("jvmCommitTransaction"));
	public static final ProgramSVSort JCABORTTRANSACTION = new SpecificMethodNameSort(
			new ProgramElementName("jvmAbortTransaction"));
	public static final ProgramSVSort JCSUSPENDTRANSACTION = new SpecificMethodNameSort(
			new ProgramElementName("jvmSuspendTransaction"));
	public static final ProgramSVSort JCRESUMETRANSACTION = new SpecificMethodNameSort(
			new ProgramElementName("jvmResumeTransaction"));

	public static final ProgramSVSort ALLOCATE = new SpecificMethodNameSort(
	...
	private static class LeftHandSideSort extends ProgramSVSort {
		...
		protected boolean canStandFor(ProgramElement pe, Services services) {
			...
			if (pe instanceof FieldReference) {
				...
				if ((fr.getProgramVariable()).isStatic()) {
					return (rp == null || rp instanceof ThisReference
							|| rp instanceof TypeReference || canStandFor(rp,
							services));
			...
			if (pe instanceof PassiveExpression) {
				return canStandFor(((NonTerminalProgramElement) pe)
		...
		protected boolean canStandFor(ProgramElement pe, Services services) {
			...
			if (pe instanceof FieldReference) {
				...
				return super.canStandFor(
						((PassiveExpression) pe).getChildAt(0), services);
			}
	...
	private static class SimpleExpressionSort extends ProgramSVSort {
		...
		protected boolean canStandFor(ProgramElement pe, Services services) {
			...
			if (pe instanceof Instanceof) {
				...
				return (v instanceof ThisReference)
						|| VARIABLE.canStandFor(v, services);
			}
	...
	private static class SpecialConstructorReferenceSort extends ProgramSVSort {
		...
		public boolean canStandFor(Term t) {
			return (t.op() instanceof ProgramMethod && !((ProgramMethod) t.op())
					.isModel());
		}
	}
	...
	private static class PureMethodBodySort extends ProgramSVSort {
		...
		protected boolean canStandFor(ProgramElement check, Services services) {
			return ((check instanceof MethodBodyStatement)
					&& ((MethodBodyStatement) check).isPure(services) && ((MethodBodyStatement) check)
	...
	private static class NonModelMethodBodySort extends ProgramSVSort {
		...
		protected boolean canStandFor(ProgramElement pe, Services services) {
			...
			final ProgramMethod pm = ((MethodBodyStatement) pe)
		...
		protected boolean canStandFor(ProgramElement pe, Services services) {
			if (pe instanceof MethodReference) {
				...
				if (mr.getReferencePrefix() != null
						&& NONSIMPLEEXPRESSION.canStandFor(mr
								.getReferencePrefix(), services)) {
					return false;
				}
	...
	public static class SimpleExpressionStringSort extends SimpleExpressionSort {
		...
		public boolean canStandFor(ProgramElement check, ExecutionContext ec,
			...
			if (check instanceof ProgramVariable) {
				...
				Sort stringSort = (Sort) ns
						.lookup(new Name("java.lang.String"));
	 ...
	 */
	private static class SimpleExpressionSpecialPrimitiveTypeSort extends
			SimpleExpressionSort {
	 ...
	 * simple expressions which have a special primitive type.
	 */
	private static class ExpressionSpecialPrimitiveTypeSort extends
			ExpressionSort {
	...
	private static class ImplicitProgramVariableSort extends LeftHandSideSort {
		...
		public boolean canStandFor(Term t) {
			return (t.op() instanceof ProgramVariable && implicit((ProgramVariable) t
					.op()));
		}
	...
	private abstract static class NameMatchingSort extends ProgramSVSort {
		...
		public NameMatchingSort(Name name, String nameStr,
		...
		public NameMatchingSort(Name name, String[] nameStrs,
				boolean ignorePrivatePrefix) {
			super(name);
	...
	private static class MethodNameReferenceSort extends NameMatchingSort {
		...
		public MethodNameReferenceSort(Name name, String methodName,
				String declaredInType) {
			super(name, methodName, false);
			this.fullTypeName = declaredInType;
		}

		public MethodNameReferenceSort(Name name, String[] methodNames,
				String declaredInType) {
		...
		public MethodNameReferenceSort(Name name, String methodName,
				String declaredInType, ListOfName signature) {
		...
		private ListOfType createSignature(Services services) {
			...
			while (ownSig.hasNext()) {
				result = result.prepend(services.getJavaInfo().getKeYJavaType(
						"" + ownSig.next()));
			}
			return result;
		}

		public boolean canStandFor(ProgramElement pe, ExecutionContext ec,
				Services services) {
			...
			if (pe instanceof MethodReference) {
				...
				if (cmpRes >= 0) {
					final KeYJavaType kjt = services.getJavaInfo()
							.getKeYJavaType(fullTypeName);
					final MethodDeclaration master = services.getJavaInfo()
							.getProgramMethod(kjt, matchingNames[cmpRes],
									createSignature(services), kjt)
							.getMethodDeclaration();
					return master == mr.method(services,
	...
	private static class ImplicitFieldSort extends FieldSort {
		...
		public ImplicitFieldSort(Name name, String field,
				boolean ignorePrivatePrefix) {
			super(name, field, ignorePrivatePrefix);
		}
		...
		protected boolean allowed(ProgramElement pe, Services services) {
			...
			return super.allowed(pe, services)
					|| (matchingNames[0] == null
							&& pe instanceof ProgramVariable && implicit(pe));
		}
	...
	private static class ImplicitFieldReferenceSort extends ImplicitFieldSort {
		...
		protected boolean allowed(ProgramElement pe, Services services) {
			return super.allowed(pe, services)
					&& ((ProgramVariable) pe).getKeYJavaType().getJavaType() instanceof ClassType;
		}
	}
	...
	private static class DLDiffSystemTopLevelOrSort extends ProgramSVSort {
		...
		public boolean canStandFor(ProgramElement pe, Services services) {
			return (pe instanceof de.uka.ilkd.key.dl.model.DiffSystem)
					&& (((de.uka.ilkd.key.dl.model.DiffSystem) pe)
							.getChildAt(0) instanceof Or);
		}
	}
	 ...
	 * @author jdq
	 */
	private static class DLDiffSystemContainsDifferentialInequalitySort extends
			ProgramSVSort {
		...
		private boolean checkInequality(ProgramElement pe, boolean inequality) {
			if (inequality && pe instanceof Dot) {
				...
				if (((PredicateTerm) pe).getChildAt(0) instanceof Less
						|| ((PredicateTerm) pe).getChildAt(0) instanceof LessEquals
						|| ((PredicateTerm) pe).getChildAt(0) instanceof GreaterEquals
						|| ((PredicateTerm) pe).getChildAt(0) instanceof Greater) {
					return (checkInequality(((PredicateTerm) pe).getChildAt(1),
							true))
							|| checkInequality(((PredicateTerm) pe)
									.getChildAt(2), true);
				}
