...
public class TermFactory {
	...
	/**
	 * class used a key for term cache if more than one composite needs to be
	 * compared Attention: for complex terms comparing may be too expensive in
	 * these cases do not cache them
	 */
	static class CacheKey {
		...
		/**
		 * the first key composite is compared by identity the second key
		 * composite is compared via equals
		 * 
		 * @param o1
		 *            the first key composite
		 * @param o2
		 *            the second key composite (non null)
		 */
		...
		/**
		 * the first key composite is compared by identity the second and third
		 * key composite is compared via equals
		 * 
		 * @param o1
		 *            the first key composite
		 * @param o2
		 *            the second key composite (non null)
		 * @param o3
		 *            the third key composite (non null)
		 */
	...
	private static Map<Object, Term> cache = Collections
			.synchronizedMap(new LRUCache<Object, Term>(5000));
	 ...
	 * 
	 * @param op
	 *            the ShadowArrayOp used to access "shadowed memory areas"
	 * @param t
	 *            the Term representing the array whose <code>index</code>
	 *            component is accessed
	 * @param index
	 *            the Term describing the index of the array component to be
	 *            accessed
	 * @param shadownum
	 *            the Term describing the nested scope of shadowed access
	 ...
	 */
	public Term createShadowArrayTerm(ShadowArrayOp op, Term t, Term index,
			Term shadownum) {
		if (op == null) {
			throw new IllegalArgumentException(
					"Creation of a shadowed array access term"
							+ "failed due to missing operator.");
		}
		return OpTerm.createOpTerm(op, new Term[] { t, index, shadownum })
				.checked();
	}

	/**
	 * creates a term representing a shadowed access on a multi dimensional
	 * array. The exact component is specified by the array of indices.
	 * 
	 * @param op
	 *            the ShadowArrayOp used to access "shadowed memory areas"
	 * @param t
	 ...
	 *            component is accessed
	 * @param index
	 *            an array of Term specifying the array component to be accessed
	 * @param shadownum
	 *            the Term describing the nested scope of shadowed access
	 * @returns the term representing a shadowed array access on the specified
	 *          component of <code>t</code> with a shadow depth of
	 *          <code>shadownum</code>
	 */
	public Term createShadowArrayTerm(ShadowArrayOp op, Term t, Term[] index,
			Term shadownum) {
	...
	/**
	 * creates a term representing an array access on the <code>index</code>-th
	 * component of <code>t</code>
	 * 
	 * @param op
	 *            the ArrayOp used to access an array of type of <code>t</code>
	 * @param t
	 *            the Term representing the array to be accessed
	 * @param index
	 *            the Term describing the index of the array component to be
	 *            accessed
	 */
	...
	/**
	 * @param arrayOp
	 *            the shadowed or normal version of the array access operator
	 * @param subs
	 *            array of subterms
	 ...
	 * 
	 * @param attrOp
	 *            the AttributeOp representing the attribute to be accessed
	 * @param subs
	 *            an array of Term containing the subterms (usually of length 1
	 *            but may have length 2 for shadowed accesses)
	 * @return the term <code>subs[0].attr</code> (or
	 *         <code>subs[0]^(subs[1]).attr)</code>)
	 */
	public Term createAttributeTerm(AttributeOp attrOp, Term[] subs) {
		if (attrOp instanceof ShadowedOperator) {
			return createShadowAttributeTerm((ShadowAttributeOp) attrOp,
					subs[0], subs[1]);
		}
	 ...
	 * 
	 * @param var
	 *            the variable the attribute term references to
	 * @param term
	 *            the Term describing the class/object of which the attribute
	 *            value has to be determined
	 * @return the attribute term "term.var"
	 */
	public Term createAttributeTerm(ProgramVariable var, Term term) {
		if (var.isStatic()) {
		...
		if (attrTerm == null) {
			attrTerm = OpTerm.createUnaryOpTerm(
					AttributeOp.getAttributeOp(var), term).checked();
	...
	public Term createAttributeTerm(SchemaVariable var, Term term) {
		return OpTerm.createUnaryOpTerm(
				AttributeOp.getAttributeOp((IProgramVariable) var), term)
				.checked();
	}
	...
	/**
	 * create an EqualityTerm with the correct equality symbol for the sorts
	 * involved, according to {@link Sort#getEqualitySymbol}
	 */
	 ...
	 * 
	 * @param op
	 *            a TermSymbol which is the top level operator of the function
	 *            term to be created
	 * @param subTerms
	 *            array of Term containing the sub terms, usually the function's
	 *            or predicate's arguments
	 * @return a term with <code>op</code> as top level symbol and the terms
	 *         in <code>subTerms</code> as arguments (direct subterms)
	 */
	...
	/**
	 * creates a term representing an OCL expression with a collection operation
	 * as top operator that takes an OclExpression as argument (not "iterate")
	 * 
	 * @param op
	 *            the OCL collection operation
	 * @param subs
	 *            subs[0] is the collection and subs[1] is the expression in
	 *            which the iterator variable is bound
	 */

	public Term createFunctionWithBoundVarsTerm(TermSymbol op,
			PairOfTermArrayAndBoundVarsArray subs) {
		return createFunctionWithBoundVarsTerm(op, subs.getTerms(), subs
				.getBoundVars());
	}

	public Term createFunctionWithBoundVarsTerm(TermSymbol op, Term[] subTerms,
	...
	public Term createIfThenElseTerm(Term condF, Term thenT, Term elseT) {
		return OpTerm.createOpTerm(Op.IF_THEN_ELSE, new Term[] { condF, thenT,
				elseT });
	}
	...
	public Term createIfExThenElseTerm(ArrayOfQuantifiableVariable exVars,
		...
		return new IfExThenElseTerm(Op.IF_EX_THEN_ELSE, new Term[] { condF,
				thenT, elseT }, exVars).checked();
	}
	 ...
	 * 
	 * @param op
	 *            Operator at the top of the termstructure that starts here
	 * @param subTerms
	 *            an array containing subTerms (an array with length 0 if there
	 *            are no more subterms
	 */
	...
	/**
	 * some methods for the creation of junctor terms with automatically
	 * performed simplification like ( b /\ true ) == (b) ... Currently only the
	 * AND, OR, IMP Operators will be simplified (if possible)
	 */
	...
	/**
	 * creates a OpTerm with a meta operator as top operator. These terms are
	 * only used in the replacewith areas of taclets. And are replaced by the
	...
	public Term createProgramTerm(Operator op, JavaBlock javaBlock, Term subTerm) {
		return new ProgramTerm(op, javaBlock, subTerm).checked();
	}

	public Term createProgramTerm(Operator op, JavaBlock javaBlock,
			Term[] subTerms) {
	 ...
	 * 
	 * @param quant
	 *            the Quantifier (all, exist) which binds the variables in
	 *            <code>varsBoundHere</code>
	 * @param varsBoundHere
	 *            an array of QuantifiableVariable containing all variables
	 *            bound by the quantifier
	 * @param subTerm
	 *            the Term where the variables are bound
	 * @return the quantified term
	 */
	public Term createQuantifierTerm(Quantifier quant,
			ArrayOfQuantifiableVariable varsBoundHere, Term subTerm) {
		if (varsBoundHere.size() <= 1) {
			return new QuantifierTerm(quant, varsBoundHere, subTerm).checked();
		} else {
			Term qt = subTerm;
			for (int i = varsBoundHere.size() - 1; i >= 0; i--) {
				QuantifiableVariable qv = varsBoundHere
	 ...
	 * 
	 * @param quant
	 *            Operator representing the Quantifier (all, exist) of this term
	 * @param var
	 *            a QuantifiableVariable representing the only bound variable of
	 *            this quantifier.
	 */
	public Term createQuantifierTerm(Quantifier quant,
			QuantifiableVariable var, Term subTerm) {
		return createQuantifierTerm(quant, new QuantifiableVariable[] { var },
				subTerm);
	}
	 ...
	 */
	public Term createQuantifierTerm(Quantifier quant,
			QuantifiableVariable[] varsBoundHere, Term subTerm) {
		return createQuantifierTerm(quant, new ArrayOfQuantifiableVariable(
				varsBoundHere), subTerm);
	}

	public Term createShadowAttributeTerm(ProgramVariable var, Term term,
			Term shadownum) {
		return OpTerm.createOpTerm(ShadowAttributeOp.getShadowAttributeOp(var),
				new Term[] { term, shadownum }).checked();
	}

	public Term createShadowAttributeTerm(SchemaVariable var, Term term,
			Term shadownum) {
		return OpTerm.createOpTerm(
				ShadowAttributeOp.getShadowAttributeOp((IProgramVariable) var),
				new Term[] { term, shadownum }).checked();
	}

	public Term createShadowAttributeTerm(ShadowAttributeOp op, Term term,
			Term shadownum) {
		return OpTerm.createOpTerm(op, new Term[] { term, shadownum })
				.checked();
	}
	...
	public Term createSubstitutionTerm(SubstOp op,
			QuantifiableVariable substVar, Term substTerm, Term origTerm) {
		return new SubstitutionTerm(op, substVar, new Term[] { substTerm,
				origTerm }).checked();
	}
	 ...
	 *            the QuantifiableVariable to be substituted
	 * @param subs
	 *            an array of Term where subs[0] is the term that replaces the
	 *            variable substVar in subs[1]
	 */
	public Term createSubstitutionTerm(SubstOp op,
	 ...
	 */
	private Term createTermWithNoBoundVariables(Operator op, Term[] subTerms,
			JavaBlock javaBlock) {
		if (op instanceof Equality) {
			...
			return createAnonymousUpdateTerm((AnonymousUpdate) op, subTerms[0]);
			...
			de.uka.ilkd.key.util.Debug.fail("Should never be"
					+ " reached. Missing case for class", op.getClass());
			return null;
		}
	}

	public Term createTerm(Operator op, Term[] subTerms,
			ArrayOfQuantifiableVariable vars, JavaBlock javaBlock) {
		if (op == null) {
			throw new IllegalArgumentException("null-Operator at TermFactory");
			...
			final ArrayOfQuantifiableVariable[] bv = new ArrayOfQuantifiableVariable[subTerms.length];
			...
			return createIfExThenElseTerm(vars, subTerms[0], subTerms[1],
					subTerms[2]);
		} else if (op instanceof SubstOp) {
			return createSubstitutionTerm((SubstOp) op, vars
					.getQuantifiableVariable(0), subTerms);
		} else {
	...
	public Term createTerm(Operator op, Term[] subTerms,
			ArrayOfQuantifiableVariable[] bv, JavaBlock javaBlock) {
		if (op == null) {
			throw new IllegalArgumentException("null-Operator at TermFactory");
		} else if (op instanceof Quantifier) {
			...
			final ArrayOfQuantifiableVariable exVars = BoundVariableTools.DEFAULT
					.unifyBoundVariables(bv, resTerms, 0, 2);
			return createIfExThenElseTerm(exVars, resTerms[0], resTerms[1],
					resTerms[2]);
		} else if (op instanceof SubstOp) {
			return createSubstitutionTerm((SubstOp) op, bv[1]
					.getQuantifiableVariable(0), subTerms);
		} else if (op instanceof TermSymbol) {
			// special treatment for OCL operators binding variables
			return createFunctionWithBoundVarsTerm((TermSymbol) op, subTerms,
					bv);
		} else {
			return createTermWithNoBoundVariables(op, subTerms, javaBlock);
		}
	 ...
	 * factory should be preferred. Depending on the needed parameters for the
	 * terms that should be created some of the parameters of this method might
	 * be ignored.
	 * 
	 * @param op
	 *            the top level operator for the new term.
	 * @param subTerms
	 *            the subterms for the new term. The first n elements are taken
	 *            if op is a Junctor or TermSymbol and n is the arity of op.
	 *            Only the first entry is taken if op is a Quantifier or a
	 *            Diamond. The first (representing the replacing term for a
	 *            variable) and the second (representing the term behind the
	 *            substitution operator) entries are taken if op is a SubstOp.
	 * @param vars
	 *            the variables that are bound to a subterm. Not considered if
	 *            op is a Junctor, TermSymbol or Diamond. If op is a SubstOp
	 *            only the first element is taken and the variable is bound to
	 *            the second subterm. In all other cases all variables are taken
	 *            and bound to the first subterm.
	 * @param javaBlock
	 *            representing a java code block. Only taken if op is a Diamond.
	 * @return the created Term
	 */
	public Term createTerm(Operator op, Term[] subTerms,
			QuantifiableVariable[] vars, JavaBlock javaBlock) {
		return createTerm(op, subTerms, new ArrayOfQuantifiableVariable(vars),
				javaBlock);
	}

	/**
	 * creates an update term like <code>{pair0}..{pairN}target</code>
	 */
	public Term createUpdateTerm(ListOfUpdatePair pairs, Term target) {
		if (pairs.size() > 1) {
			return createUpdateTerm(pairs.head(), (createUpdateTerm(pairs
					.tail(), target)));
		} else {
	 ...
	 * 
	 * @param loc
	 *            the Term representing the location to be updated
	 * @param value
	 *            the Term representing the value the location is updated to
	 * @param target
	...
	public Term createUpdateTerm(Term loc, Term value, Term target) {
		return createUpdateTerm(new Term[] { loc }, new Term[] { value },
				target);
	}
	 ...
	 */
	public Term createUpdateTerm(Term[] locs, Term[] values, Term target) {
		final ArrayOfQuantifiableVariable[] boundVars = new ArrayOfQuantifiableVariable[locs.length];
	...
	public Term createUpdateTerm(UpdatePair pair, Term target) {
		...
		if (op instanceof QuanUpdateOperator) {
			final ArrayOfQuantifiableVariable[] boundVars = new ArrayOfQuantifiableVariable[pair
					.arity() + 1];
			for (int i = 0; i < subs.length - 1; i++)
				boundVars[i] = pair.varsBoundHere(i);
			boundVars[subs.length - 1] = new ArrayOfQuantifiableVariable();
			return createQuanUpdateTerm((QuanUpdateOperator) op, subs,
					boundVars);
		} else {
	 ...
	 */
	public Term createNormalizedQuanUpdateTerm(QuanUpdateOperator op,
			Term[] subs, ArrayOfQuantifiableVariable[] boundVarsPerSub) {
	 ...
	 *            the sub-terms
	 */
	public Term createQuanUpdateTerm(QuanUpdateOperator op, Term[] subs,
			ArrayOfQuantifiableVariable[] boundVarsPerSub) {
		final ArrayOfQuantifiableVariable[] boundVars = op
	 ...
	 */
	public Term createQuanUpdateTermUnordered(QuanUpdateOperator op,
			Term[] subs, ArrayOfQuantifiableVariable[] boundVars) {
	...
	public Term createQuanUpdateTerm(ArrayOfQuantifiableVariable[] boundVars,
			Term[] guards, Term[] locs, Term[] values, Term target) {

		return QuanUpdateOperator.normalize(boundVars, guards, locs, values,
				target);
	}
	 ...
	 * 
	 * @param v
	 *            the ProgramVariable to be represented
	 ...
	 * 
	 * @param v
	 *            the SchemaVariable to be represented
	 ...
	 * 
	 * @param op
	 *            the AnonymousUpdate operator
	 * @param sub
	 *            the Term the anonymous update is applied to
	 * @return the created term
	 */
	public Term createAnonymousUpdateTerm(AnonymousUpdate op, Term sub) {
	...
	public Term createAnonymousUpdateTerm(Name name, Term target) {
		return createAnonymousUpdateTerm(AnonymousUpdate
				.getAnonymousOperator(name), target);
	}
