...
public class TacletInstantiationsTableModel extends AbstractTableModel {
	 ...
	 */
	public TacletInstantiationsTableModel(TacletApp app, Services services,
			NamespaceSet nss, AbbrevMap scm, Goal goal) {
	...
	private Vector createEntryArray(TacletApp tacletApp) {
		...
		while (it.hasNext()) {
			...
			column[0] = sv;
			column[1] = ProofSaver.printAnything(tacletApp.instantiations()
					.getInstantiation(sv), services);
			rowVec.add(column);
			count++;
		}
		...
		IteratorOfSchemaVariable varIt = tacletApp.uninstantiatedVars()
				.iterator();
	 ...
	 */
	public IdDeclaration parseIdDeclaration(String s) throws ParserException {
		try {
			...
			throw new ParserException(re.getMessage(), new Location(re
					.getFilename(), re.getLine(), re.getColumn()));
	...
	private void checkNeededInputAvailable(int irow)
		...
		if ((getValueAt(irow, icol) == null || ((String) getValueAt(irow, icol))
				.length() == 0)
				&& !originalApp.sufficientlyComplete()
				&& !originalApp.canUseMVAPriori((SchemaVariable) getValueAt(
						irow, 0))) {
	...
	private boolean isInputAvailable(int irow) {
		if (((SchemaVariable) getValueAt(irow, 0)).isListSV())
			return true;
		return getValueAt(irow, 1) != null
				&& ((String) getValueAt(irow, 1)).length() != 0;
	}

	private SetOfLocationDescriptor parseLocationList(int irow)
			throws ParserException {
		...
		try {
			result = (new KeYParser(ParserMode.TERM, new KeYLexer(
					new StringReader(instantiation), services
							.getExceptionHandler()), null, TermFactory.DEFAULT,
					null, services, nss, scm)).location_list();
		} catch (antlr.RecognitionException re) {
			throw new ParserException(re.getMessage(), new Location(re
					.getFilename(), re.getLine(), re.getColumn()));
		} catch (antlr.TokenStreamException tse) {
		...
		try {
			...
			if (loc != null) {
				throw new SVInstantiationParserException(instantiation, irow
						+ (loc.getLine() <= 0 ? 0 : loc.getLine()), loc
						.getColumn(), pe.getMessage(), false);
			} else {
				throw new SVInstantiationParserException(instantiation, irow,
		...
		if (svSort == ProgramSVSort.LABEL) {
			...
			if (nvc != null) {
				...
				if (o instanceof SchemaVariable) {
					...
					final Object peerInst = app.instantiations()
					...
					kjt = tc
							.getKeYJavaType(peerInstExpr, app.instantiations()
	...
	private ProgramElement parseRow(int irow)
		...
		ProgramElement pe = getProgramElement(originalApp, instantiation, sv,
				services);
		if (pe == null) {
		...
		try {
			...
			for (irow = noEditRow + 1; irow < entries.size(); irow++) {
				...
				if (sv.isVariableSV() || sv.isSkolemTermSV()) {
					...
					if (sv.isVariableSV()) {
						LogicVariable lv = new LogicVariable(new Name(idd
								.getName()), sort);
						...
						Named n = namespaces().lookupLogicSymbol(
								new Name(idd.getName()));
						if (n == null) {
							result = result.createSkolemConstant(idd.getName(),
									sv, sort, true);
						} else {
							throw new SVInstantiationParserException(idd
									.getName(), irow, 1,
			...
			for (irow = noEditRow + 1; irow < entries.size(); irow++) {
				...
				if (result.isDependingOnModifiesSV(sv) || sv.isVariableSV()
						|| sv.isSkolemTermSV()
				...
				if (sv.isProgramSV()) {
					...
					try {
						...
						result = result.addInstantiation(sv, Array.reverse(s
								.toArray()), true);
					} catch (ParserException pe) {
						Location loc = pe.getLocation();
						if (loc != null) {
							throw new SVInstantiationParserException(
									(String) getValueAt(irow, 1), irow
											+ (loc.getLine() <= 0 ? 0 : loc
													.getLine()), loc
											.getColumn(), pe.getMessage(),
									false);
						} else {
							throw new SVInstantiationParserException(
									(String) getValueAt(irow, 1), irow, -1, pe
											.getMessage(), false);
						}
					}
				} else if (sv.isNameSV()) {
					...
					if (!result.isDependingOnModifiesSV(sv)
							&& isInputAvailable(irow)) {
						final Namespace extVarNS = result
								.extendVarNamespaceForSV(nss.variables(), sv);

						Namespace functNS = result
						...
						try {
							...
							throw new SVInstantiationParserException(
									(String) getValueAt(irow, 1), irow, -1, iae
											.getMessage(), false);
						}
					}
				}
	...
	public static String getNameProposalForMetavariable(Goal goal,
			TacletApp p_app, SchemaVariable p_var) {
		...
		if (s.contains("_")) {
			...
			try {
				...
				String newName = s.substring(0, s.lastIndexOf('_'));
				while (Main.getInstance().mediator().getServices()
						.getNamespaces().lookup(new Name(newName + ++i)) != null)
					;
				return newName + i;
			} catch (Exception e) {
			}
