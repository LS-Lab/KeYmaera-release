...
public class KeYFile implements EnvInput {
     ...
     * 
     * @param monitor
     *            the progress monitor to notify on the state of reading
     */
    ...
    /**
     * creates a new representation for a given file by indicating a name and a
     * RuleSource representing the physical source of the .key file.
     * 
     * @param monitor
     *            the progress monitor to notify on the state of reading
     */
    ...
    /**
     * returns the initial configuration that is used to read and that may be
     * modified by reading.
     */
    ...
    protected InputStream getNewStream() throws FileNotFoundException {
        ...
        if (!file.isAvailable()) {
            throw new FileNotFoundException("File/Resource " + file
                    + " not found.");
        }
    ...
    public void read(ModStrategy mod) throws ProofInputException {
        ...
        try {
            ...
            final ParserConfig normalConfig = new ParserConfig(initConfig
                    .getServices(), normal);
            final ParserConfig schemaConfig = new ParserConfig(initConfig
                    .getServices(), schema);
            ...
            problemParser = new KeYParser(ParserMode.PROBLEM, new KeYLexer(
                    cinp, initConfig.getServices().getExceptionHandler()), file
     ...
     * namespaces. Therefore we take the given namespaces and use copies of the
     * normal function and variables namespace TODO: extend the normal namespace
     * by a generic sort and schema function namespace and get rid of the
     * schemaConfig...
     * 
     * @param normal
     *            the Namespace containing the concrete symbols
     * @return namespace for reading in rules etc.
     */
    protected NamespaceSet setupSchemaNamespace(final NamespaceSet normal) {
        return new NamespaceSet(normal.variables().copy(), normal.functions()
                .copy(), normal.sorts(), normal.ruleSets(), normal.choices(),
    ...
    public Includes readIncludes() throws ProofInputException {
        if (file == null)
            return new Includes();
        try {
            final ParserConfig pc = new ParserConfig(new Services(),
                    new NamespaceSet());
            // FIXME: there is no exception handler here, thus, when parsing
            // errors are ecountered
            // during collection of includes (it is enough to mispell \include)
            // the error
            ...
            ProgramBlockProvider provider = profile.getProgramBlockProvider();
            problemParser = new KeYParser(ParserMode.PROBLEM, new KeYLexer(
                    getNewStream(), null), file.toString(), pc, pc, null, null,
                    null, provider);
    ...
    public LibrariesSettings readLibrariesSettings() throws ProofInputException {
        ...
        if (settings == null
                || settings.getLibrariesSettings().emptyProperties()) {
    ...
    private KeYParser createDeclParser() throws FileNotFoundException {
        ...
        return new KeYParser(ParserMode.DECLARATION,
                new KeYLexer(getNewStream(), initConfig.getServices()
                        .getExceptionHandler()), file.toString(), initConfig
                        .getServices().copy(), initConfig.namespaces().copy(),
                provider);
    }

    /**
     * reads the sorts declaration of the .key file only, modifying the sort
     * namespace of the initial configuration if allowed in the given
    ...
    /**
     * reads the rules and problems declared in the .key file only, modifying
     * the set of rules of the initial configuration if allowed in the given
     * modification strategy.
     */
    public void readRulesAndProblem(ModStrategy mod) throws ProofInputException {

        /*
         * two namespace sets which share all namespace except the variable and
         * function namespace
         */
        final NamespaceSet normal = initConfig.namespaces().copy();
        final NamespaceSet schema = setupSchemaNamespace(normal);
    ...
    public ProofSettings getPreferences() throws ProofInputException {
        if (settings == null) {
            ...
            try {
                ...
                problemParser = new KeYParser(ParserMode.PROBLEM, new KeYLexer(
                        getNewStream(), null), file.toString(), provider);
                settings = new ProofSettings();
                settings
                        .setProfile(ProofSettings.DEFAULT_SETTINGS.getProfile());
        ...
        try {
            ...
            if (javaPath.length() != 0) {
                ...
                if (!cfile.isAbsolute()) { // test relative pathname
                    ...
                    cfile = new File(parent, javaPath).getCanonicalFile()
                            .getAbsoluteFile();
