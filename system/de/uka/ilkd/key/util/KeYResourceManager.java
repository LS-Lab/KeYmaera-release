...
public class KeYResourceManager {
	...
	private String readVersionString(URL url) {
		...
		if (url != null) {
			try {
				final InputStream io = new BufferedInputStream(url.openStream());
				int c;
				while ((c = io.read()) != -1) {
					result += (char) c;
				}
	 ...
	 * found there the packagename of its superclass is used recusrivly.
	 * 
	 * @param o
	 ...
	 * is not found the packagename of its superclass is used recursivly.
	 * 
	 * @param cl
	...
	/**
	 * Copies the specified resource to targetLocation if such a file does not
	 * exist yet. The created file is removed automatically after finishing
	 * JAVA.
	 * 
	 * @param o
	 *            an Object the directory from where <code>resourcename</code>
	 *            is copied is determined by looking on the package where
	 *            <code>o.getClass()</code> is declared
	 * @param resourcename
	 *            String the name of the file to search (only relative pathname
	 *            to the path of the calling class)
	...
	public boolean copyIfNotExists(Class cl, String resourcename,
		...
		if (resourceURL == null && cl.getSuperclass() != null) {
			return copyIfNotExists(cl.getSuperclass(), resourcename,
					targetLocation);
