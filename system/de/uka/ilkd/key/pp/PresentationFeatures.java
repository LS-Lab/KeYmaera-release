...
public class PresentationFeatures {
	...
	private static void putInfixNotation(NotationInfo notInfo,
			Namespace functions, String op, String token) {
		notInfo.createInfixNotation((Function) functions.lookup(new Name(op)),
				token);
	}

	private static void putInfixNotation(NotationInfo notInfo,
			Namespace functions, String op, String token, int prio, int lass,
			int rass) {
		notInfo.createInfixNotation((Function) functions.lookup(new Name(op)),
				token, prio, lass, rass);
	}

	private static void putPrefixNotation(NotationInfo notInfo,
			Namespace functions, String op, String token) {
		notInfo.createPrefixNotation((Function) functions.lookup(new Name(op)),
				token);
	}
	...
	public static void modifyNotationInfo(NotationInfo notInfo,
		...
		if (ENABLED) {
			...
			putInfixNotation(notInfo, functions, "equals", "=", 80, 90, 90);
			putInfixNotation(notInfo, functions, "neq", "!=", 80, 90, 90);
			...
			putInfixNotation(notInfo, functions, "exp", "^", 150, 151, 109);
			putPrefixNotation(notInfo, functions, "neg", "-");

			notInfo.createNumLitNotation((Function) functions.lookup(new Name(
					"Z")));
			...
			notInfo.createOCLCollectionNotation("insert_collection",
					"Collection");
	...
	public static String printNumberTerm(Term t) {
		...
		if (t.sub(0).op().name().toString().equals("neglit")) {
			return "-" + printlastfirst(t.sub(0).sub(0)).toString();
		} else {
