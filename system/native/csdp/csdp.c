...
double* convert_jdoubleArray_to_double_range(JNIEnv* env, jdoubleArray array, int from, int count) {
...
struct constraintmatrix* convert_double_array_to_constraintmatrix(JNIEnv* env, int n, int k, jdoubleArray constraints) {
...
#define INDICES_TYPE int
	...
	for(i=1; i <= k; i++) {
		block->entries = convert_jdoubleArray_to_double_range(env, constraints, (i-1)*k, n);
															...
															pdobj)
{
	...
	struct constraintmatrix* _constraints = convert_double_array_to_constraintmatrix(env, (int)n, (int)k, constraints);
